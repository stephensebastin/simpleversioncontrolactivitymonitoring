...
async function processPush(pushInfo) {
    ...
    try{       
        ...
        const result = await sequelize.transaction(async (t) => {
            ...
            if(valueFromRedis != null && (valueFromRedis == true || valueFromRedis == 'true')) {
             ...
             }*/ //check this with worker thread to test concurrent check
    ...
    //get user id and branch id
    //check for redis lock

    //if already locked wait for some time for lock relaese on redis
    // once lock is released check for latest update time stamp
    // if time stamp is greater with commited time stamp conflict return 
    // else proceed for inserting commits
}
...
async function checkFileExist( fileName) { //check if file exists and returns fileid if exists else returns null
    try {
        ...
        /* if(fileInfoObj == null){
           var file = await createFile(branchid,fileName);
            return await fileinfo.findOne({where : {filename:fileName}}); 
        } */
